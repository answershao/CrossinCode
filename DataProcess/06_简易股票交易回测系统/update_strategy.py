...
def simple_test(currency, stocks):
    ...
    if current_date in date_index[:3]:
        ...
        for stock in stocks.keys():
            ...
            # 连续上涨3天
            if (currency >= current_stock.iloc[location]) and (current_stock.iloc[location] > current_stock.iloc[location-1] > current_stock.iloc[location-2] > current_stock.iloc[location-3]):
                amount = int(currency / current_stock.iloc[location])
                currency += purchase(stock, amount, currency, stocks)
            # 连续下跌2天
            elif current_stock.iloc[location] < current_stock.iloc[location-1] < current_stock.iloc[location-2]:
                amount = stocks[stock]
...
def multi_stocks_test(currency, stocks):
    """
    测试用策略
    持有多只股票的情况
    ...
    # 产生交易
        ...
        for stock in stocks.keys():
            current_stock = stock_data[stock]
            # 连续上涨两天
            if (currency > current_stock.iloc[location]) and (current_stock.iloc[location] > current_stock.iloc[location-1] > current_stock.iloc[location-2]):
...
def yield_rate(stock):
    ...
    # 检查在period 时间段之前的日期是否为交易日， 如果不是的话，向前迭代
